<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\PicturesTag $picturesTag
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Pictures Tag'), ['action' => 'edit', $picturesTag->picture_id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Pictures Tag'), ['action' => 'delete', $picturesTag->picture_id], ['confirm' => __('Are you sure you want to delete # {0}?', $picturesTag->picture_id)]) ?> </li>
        <li><?= $this->Html->link(__('List Pictures Tags'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Pictures Tag'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Pictures'), ['controller' => 'Pictures', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Picture'), ['controller' => 'Pictures', 'action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Tags'), ['controller' => 'Tags', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Tag'), ['controller' => 'Tags', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="picturesTags view large-9 medium-8 columns content">
    <h3><?= h($picturesTag->picture_id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Picture') ?></th>
            <td><?= $picturesTag->has('picture') ? $this->Html->link($picturesTag->picture->id, ['controller' => 'Pictures', 'action' => 'view', $picturesTag->picture->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Tag') ?></th>
            <td><?= $picturesTag->has('tag') ? $this->Html->link($picturesTag->tag->title, ['controller' => 'Tags', 'action' => 'view', $picturesTag->tag->id]) : '' ?></td>
        </tr>
    </table>
</div>
